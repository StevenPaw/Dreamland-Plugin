...
public class CMDgamemode implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(sender instanceof Player) {
			...
			if(p.hasPermission("dreamland.gamemode.*") || p.hasPermission("dreamland.*") || p.hasPermission("dreamland.gamemode.self")) {
				if(args.length == 1) {
			...
			if(p.hasPermission("dreamland.gamemode.*") || p.hasPermission("dreamland.*") || p.hasPermission("dreamland.gamemode.others")) {
				if(args.length == 2) {
